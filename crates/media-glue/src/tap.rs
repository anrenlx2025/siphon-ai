...
pub struct MediaTap {
    ...
    /// emits one frame toward the caller per 20 ms tick whenever the
    /// outbound queue is empty and no other feature owns the caller's
    /// ear (park/hold/announcement, pending barge-in arbitration, mute,
    /// room membership, a dropped WS, the #417 tx gate). Default `Off`;
...
impl MediaTap {
    ...
    pub async fn run(
        ...
        let keepalive_frame_samples = (self.sample_rate / 1000) as usize * 20;
        ...
        loop {
            tokio::select! {
                ...
                // peer RTP dries up, starving the call. The guard
                // mirrors the pace tick's philosophy: with the feature
                // off (the default) the arm is never selectable and
                // the idle call pays nothing. The suppression set
                // tracks the pace tick's `feed_ok` conditions — park,
                // hold, announcement, pending arbitration, mute, a
                // dropped WS, room membership — plus the empty
                // outbound queue that defines "idle".
