...
— and, since 0.51.0, `siphon_ai_hep_collector_up` drops to `0` for the
sample interval in which a send was refused, with the refused packets
counted under `siphon_ai_hep_packets_dropped_total{reason="collector_down"}`
(#596). `docs/HEP.md` has the full diagnostic order, including the
black-holed-collector case UDP cannot report.
...
| `siphon_ai_hep_packets_sent_total`      | counter   | —                                     | HEP3 packets written to the wire (`[hep]`). Mirrored every 10 s from the sink's own counter, so it lags a scrape slightly; present from startup. **Wire-level success only, and not a liveness signal** — a black-holing NAT or a heplify-server that isn't storing still counts here, and against a *dead* collector the counter keeps climbing at roughly half rate (connected-UDP reports the refusal on the following send, so alternate sends succeed). Alert on `siphon_ai_hep_collector_up` for that; see `docs/HEP.md`. |
| `siphon_ai_hep_packets_dropped_total`   | counter   | `reason=queue_full\|collector_down`   | HEP3 packets that never reached the collector. `queue_full`: the producer dropped it because `[hep].queue_capacity` (default 256) was full — the daemon emitting faster than the worker ships, per CLAUDE.md §4.7; sustained movement means raise the capacity, it does **not** mean the collector is down. `collector_down` (0.51.0, #596): the worker dequeued it but the socket refused the send — `ECONNREFUSED` after a dead collector's ICMP port-unreachable. Every packet handed to the sink lands in exactly one of `sent` / `queue_full` / `collector_down`. Both label values present from startup at 0. |
| `siphon_ai_hep_collector_up`            | gauge     | —                                     | `1` unless a HEP send was refused during the last 10 s sample interval, then `0` (0.51.0, #596 — the gauge CLAUDE.md §4.7 always described). Derived over the window rather than from the last send because connected UDP alternates success/failure against a dead peer. **Alert on `== 0`.** UDP caveat: a collector that is black-holed (no ICMP back) or up-but-discarding looks up here — see `docs/HEP.md` for the checks that cover those. |
| `siphon_ai_hep_node_events_total`       | counter   | `event=node_started\|node_ready\|node_not_ready\|node_draining\|node_status\|node_stopping` | Node-health HEP `Log` chunks handed to the sink (0.52.0; `[hep]` with a collector) — see `docs/HEP.md` → *Node health Log chunks*. Counts emission, not delivery (that is `siphon_ai_hep_packets_*`). `node_status` rises once per `[hep].node_status_interval_secs`; each label value appears when it first fires. |
| `siphon_ai_otlp_log_records_dropped_total` | counter | `reason=queue_full\|collector_down`  | Log records that never reached the OTLP collector (`[observability.otlp.logs]`, 0.51.0). `queue_full`: the emitting task hands each record to a bounded queue and returns; a worker thread drains it into the SDK, and a full queue means the export path is not keeping up with the log rate, so the record is discarded rather than allowed to hold up the task that raised it (CLAUDE.md §4.7) — sustained movement usually means `[observability.otlp.logs].level` is too broad for the link. `collector_down` (#596): the SDK's batch export to the collector failed (connection refused, gRPC timeout, non-OK status) and the whole batch was discarded — counted by the exporter wrapper, the only layer that sees the failure; `queue_full` stays at 0 through such an outage because the SDK accepts records instantly and loses them one hop later (the 0.51.0 soak ran 46 minutes that way). **Movement on either means the console has lines the collector never received.** |
| `siphon_ai_otlp_spans_dropped_total`    | counter   | `reason=collector_down`               | Spans the OTLP batch exporter failed to deliver (`[observability.otlp]`, 0.51.0, #596) — the span-side twin of the `collector_down` reason above; one failed batch counts every span in it. Present from startup at 0. |
| `siphon_ai_otlp_collector_up`           | gauge     | —                                     | `1` if the last OTLP batch export (spans or log records — they share the endpoint) succeeded, `0` if it failed (0.51.0, #596). Set on every export, so it reflects the collector as of the most recent batch (~1 s under load, longer when idle) and recovers on the first success after the collector returns. Published as `1` when the pipeline is built. **Alert on `== 0`** — this is the "is the collector gone?" question the drop counters cannot answer. |
