...
the admission math stay coherent. Three consequences worth knowing:

- **The socket binds inside `rtp_port_range`**, so the firewall rule
  you already opened for RTP covers browser media too — no second
  range, and no ephemeral port outside your rules.
- **`siphon_ai_rtp_port_pairs_allocated` counts browser calls**, so
  the gauge you watch for capacity does not under-report when part of
  your traffic is browsers.
- **A browser call is refused by `reserved_outbound_calls`** exactly
  as a SIP call is — `503` + `Retry-After`, counted in
  `siphon_ai_rtp_reserve_blocks_total`. It holds a whole pair despite
  using one port, because it occupies one call slot; charging it half
  a slot would make the reservation mean two different things
